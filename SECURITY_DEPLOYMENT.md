...
6. **Turnstile : renseigner `TURNSTILE_SITE_KEY` et `TURNSTILE_SECRET_KEY`**, autoriser les domaines du frontend et vérifier une validation réelle. Le parcours gratuit, connecté ou anonyme, exige une nouvelle validation après les publicités. Le serveur rattache cette validation à l'autorisation du contenu et à la session : un ancien pass ne la remplace pas. Premium reste exempté. Une panne bloque ce parcours avec possibilité de réessayer; aucun jeton de secours n'est accepté.
