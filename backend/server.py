...
def safe_int(value, default=0) -> int:
    """Conversion numérique qui ne plante jamais : une valeur invalide
    (texte, None, etc.) retombe simplement sur la valeur par défaut au
    lieu de faire échouer tout l'appel qui l'entoure."""
    try:
        return int(value)
    except (TypeError, ValueError):
        return default
...
def serialize_admin_media(doc) -> dict:
    return {
        ...
        "reports_open": safe_int(doc.get("reports_open"), 0),
...
async def _uqflex_media_docs(type_filter: Optional[str] = None, q: Optional[str] = None) -> list:
    ...
    episodes_map = {e["id"]: e async for e in db.uqflex_episode_cache.find({"id": {"$in": ids}}, {"_id": 0})}
    ...
    for item in items:
        ...
        if q_lower and q_lower not in (doc.get("title") or "").lower():
            continue
        episodes = episodes_map.get(media_id)
        if episodes and episodes.get("seasons"):
            doc["seasons"] = episodes["seasons"]
        enrich = enrich_map.get(media_id)
        if enrich:
            for champ in ("cast", "director", "trailer_youtube_id", "saga_title", "timeline", "title_logo_url", "tmdb_id", "poster_url", "banner_url"):
...
async def list_media(type: Optional[str] = None, q: Optional[str] = None, featured: Optional[bool] = None, limit: int = Query(100, ge=1, le=5000), uqflex: Optional[bool] = None):
    ...
    for d in docs:
        try:
            resultats.append(serialize_media(d))
        except Exception:
            # Un titre malformé (ex. import externe avec un champ invalide)
            # ne doit jamais faire échouer tout le listing : on l'ignore
            # simplement plutôt que de faire disparaître toute la page.
            logger.exception("serialize_media a échoué pour %s", d.get("id"))
    return resultats
        ...
        for mid in ordered_ids:
            ...
            if d:
                try:
                    ...
                    logger.exception("serialize_media a échoué pour %s", mid)
                else:
...
async def _uqflex_status_payload() -> dict:
    ...
    if items:
        ...
        if series_ids:
            ...
            pending_episodes = max(0, len(series_ids) - resolus)
    return {
    ...
    for f in favs:
        ...
        if d:
            try:
                ...
                logger.exception("serialize_media a échoué pour %s", f["media_id"])
                continue
...
PALIERS_DEFAUT = [
    ...
    {
        "amount": 100,
        "label": "Dix gagnants d'un coup",
        "rewards": [
            "5 abonnements Standard d'un mois",
            "5 abonnements Basic d'un mois",
        ],
        "highlight": False,
    },
    {
    ...
    {
        "amount": 500,
        "label": "Une année offerte",
        "rewards": [
            "3 abonnements Premium d'un an",
            "15 abonnements Standard d'un mois",
        ],
        "highlight": False,
    },
    {
        "amount": 750,
        "label": "Le grand tirage",
        "rewards": [
            "50 € en cash à gagner sur le Discord",
            "5 abonnements Premium d'un an",
            "20 abonnements Standard d'un mois",
        ],
        "highlight": False,
    },
    {
        "amount": 1000,
        "label": "Le cashprize final",
        "rewards": [
            "150 € en cash à gagner sur le Discord",
            "Un abonnement Premium à vie",
            "10 abonnements Premium d'un an",
        ],
        "highlight": False,
    },
]
...
async def list_progress(user: dict = Depends(get_current_user), profile_id: Optional[str] = Depends(current_profile_id)):
    ...
    for d in docs:
        ...
        try:
            ...
            logger.exception("serialize_media a échoué pour %s", d["media_id"])
            continue
...
async def recommendations(limit: int = 20, user: dict = Depends(get_current_user), profile_id: Optional[str] = Depends(current_profile_id)):
    ...
    def recommendation_score(item):
        ...
        score += safe_float(item.get("rating"), 0.0)
        return score
    ranked = sorted(candidates, key=recommendation_score, reverse=True)
    resultats = []
    for item in ranked:
        if recommendation_score(item) <= 0:
            continue
        try:
            resultats.append(serialize_media(item))
        except Exception:
            logger.exception("serialize_media a échoué pour %s", item.get("id"))
...
async def similar_media(media_id: str, limit: int = 8):
    ...
    def score(c):
        ...
        rating_boost = safe_float(c.get("rating"), 0.0) / 20
    ...
    for c in cands:
        if len(resultats) >= limit:
            break
        try:
            resultats.append(serialize_media(c))
        except Exception:
            logger.exception("serialize_media a échoué pour %s", c.get("id"))
    return resultats
